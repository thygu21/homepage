...
try{
	...
	if(cookies!=null){
		for(int i=0; i<cookies.length; i++){                        // 쿠키를 반복문으로 돌린다.
			if(cookies[i].getName().equals("who")){            // 쿠키의 이름이 id 일때
				who=cookies[i].getValue();                        // 해당 쿠키의 값을 id 변수에 저장한다.
			}
		}
	}
}catch(Exception e){} %>
...
<div class="container">
    ...
    <div class="col-md-9">
        ...
        <form class="form-inline"  name="ganguisearch" method="get" action="gangui.jsp">
            ...
            <button type="submit" class="btn btn-default">강의과목 조회</button>
        </form>
		<%
		String professor = who.replace("professor", "교수");   
		...
		Statement stmt = null;
		ResultSet rst = null;
		String query = "SELECT * FROM lecture WHERE professor='" + professor + "' AND year=" + year + " AND semester='" + semester + "';";
		try {
		...
		if (rst.next())
		{
			if(who.matches(".*professor.*")){
				...
				out.println("<strong>" + year + "년도 " + semester + " 대학 강의과목</strong>");
				out.println("</div><table class=\"table table-bordered\"><thead><tr><th>번호</th><th>과목명</th><th>담당교수</th><th>학정번호</th><th>인원</th><th>수강생조회</th><th>강의자료실</th></tr></thead><tbody>");			
