...
<div class="container">
    ...
    <div class="col-md-9">
        ...
        <form class="form-inline"  name="lectsearch" method="get" action="lect.jsp">
            ...
            <button type="submit" class="btn btn-default">강의계획서 조회</button>
            <%
            if(who.matches(".*professor.*")){
				out.println("<a href=\"lectinput.jsp\" class=\"btn btn-success form-r\" role=\"button\">강의계획서 입력</a>");
			}
            %>
        </form>
		<%		
		...
		Statement stmt = null;
		ResultSet rst = null;
		String query = "SELECT * FROM assignment3.lecture WHERE year=" + year + " AND semester='" + semester + "';";
		try {
		...
		if (rst.next())
		{
			if(who.matches(".*professor.*")){
				...
				out.println("<strong>" + year + "년도 " + semester + " 대학 강의계획서</strong>");
				out.println("</div><table class=\"table table-bordered\"><thead><tr><th>번호</th><th>과목명</th><th>담당교수</th><th>학정번호</th><th>인원</th></tr></thead><tbody>");			
				while (rst.next()) { // ResultSet 객체를 한줄 마다 불러온다
					out.println("<tr><td>" + rst.getRow() + "</td><td>" + rst.getObject(1) + "</td><td>" + rst.getObject(2) + "</td><td>" + rst.getObject(3) + "</td><td>" + rst.getObject(4) + "</td></tr>");
				}
				...
				out.println("<div class=\"panel panel-default margin-top\"><div class=\"panel-heading\">");
				out.println("<strong>" + year + "년도 " + semester + " 대학 강의계획서</strong>");
				out.println("</div><table class=\"table table-bordered\"><thead><tr><th>번호</th><th>과목명</th><th>담당교수</th><th>학정번호</th><th>인원</th><th>수강신청</th></tr></thead><tbody>");			
				while (rst.next()) { // ResultSet 객체를 한줄 마다 불러온다
					out.println("<tr><td>" + rst.getRow() + "</td><td>" + rst.getObject(1) + "</td><td>" + rst.getObject(2) + "</td><td>" + rst.getObject(3) + "</td><td>" + rst.getObject(4) + "</td>" +
					"<td><form name=\"sugang\" method=\"post\" action=\"sugang_post.jsp\">" +
                	"<input type=\"hidden\" name=\"lecture_num\" value=\"" + rst.getObject(3) + "\">" +
                	"<button type=\"submit\" class=\"btn btn-default btn-xs\">신청</button>" +
            		"</form></td></tr>");
				}
				out.println("</tbody></table></div>");
			}
